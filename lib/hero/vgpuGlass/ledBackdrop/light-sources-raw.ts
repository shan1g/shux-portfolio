...
function ledEmitterVertexData(
  ...
  // Each edge of the stretched triangle packs its LEDs from its own length, so
  // the emitter quad length comes from the LED rather than from a single shared
  // `ledShape.tangentHalfLength` (kept as the fallback).
  const paddedHalfLengthOf = (led: { halfLength?: number }) =>
    (led.halfLength ?? tangentHalfLength) + pad;
  const values: number[] = [];
  ...
  for (const [ledIndex, led] of layout.positions.entries()) {
    ...
    const paddedHalfLength = paddedHalfLengthOf(led);
    pushQuad(
      ledIndex,
      led,
      basis.dir,
  ...
  for (let edge = 0; edge < 3; edge++) {
    ...
    // The two edges meeting at this corner can now carry different LED lengths,
    // so the seam is built from each side's own half-length.
    const incomingPaddedHalfLength = paddedHalfLengthOf(incoming);
    const outgoingPaddedHalfLength = paddedHalfLengthOf(outgoing);
    const incomingEnd = {
      x: incoming.x + incomingBasis.dir.x * incomingPaddedHalfLength,
      y: incoming.y + incomingBasis.dir.y * incomingPaddedHalfLength,
    };
    const outgoingStart = {
      x: outgoing.x - outgoingBasis.dir.x * outgoingPaddedHalfLength,
      y: outgoing.y - outgoingBasis.dir.y * outgoingPaddedHalfLength,
    };
    ...
    const seamDistance =
      ...
      (incomingPaddedHalfLength + outgoingPaddedHalfLength) * 0.5;
    const seam = {
      x: corner.x + inwardBisector.x * seamDistance,
      y: corner.y + inwardBisector.y * seamDistance,
    };
