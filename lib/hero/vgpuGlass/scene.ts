...
import type { LedBackdrop } from "./ledBackdrop/createLedBackdrop";
...
export const HERO_FLOOR_AO_DEFAULTS = {
  // The glass floor-AO footprint is an analytic triangle in the XZ plane, so it
  // narrows with the hull's in-plane stretch. X and Z are stretched equally, so
  // the footprint stays equilateral and only its scale changes.
  glassAoScale: 0.54 * HERO_GLASS_HULL_STRETCH.x,
  glassAoAmplitude: 0.41,
  glassAoOpacity: 0.11,
  fractalAoScale: 0.88,
  fractalAoAmplitude: 0.32,
  fractalAoOpacity: 0.28,
  orbAoScale: 0.58,
  orbAoAmplitude: 0.59,
  orbAoOpacity: 0.73,
};
...
export interface HeroFractalScene {
  ...
  readonly ledFloorTarget: Target;
  readonly sceneSampler: GPUSampler;
  readonly ledFloorSampler: GPUSampler;
...
export function setHeroFractalSceneSettings(
  ...
  scene.background.set({
    ...
    ledFloorTexture: scene.ledFloorTarget,
    ledFloorSampler: scene.ledFloorSampler,
  });
  const glassParams = {
...
export function renderHeroFractalScene(
  ...
  scene: HeroFractalScene,
  options: {
    ledBackdrop?: LedBackdrop;
    time?: number;
    finalDebugDraws?: readonly Draw[];
  } = {}
): void {
  const { ledBackdrop, time = 0, finalDebugDraws = [] } = options;
  frame(gpu, (currentFrame) => {
    ledBackdrop?.renderFrame(currentFrame, { time });
    currentFrame.pass(
      {
