...
// Non-uniform stretch that turns the authored beveled tetrahedron into a tall
// gem. Authored in `lib/hero/vgpuGlass/hullShape.ts`; WGSL cannot import a
// TypeScript value, so keep this in sync with that file and with
// `hero-glass.wgsl`, which shades the same geometry.
const HULL_STRETCH = vec3f(0.82, 1.34, 0.82);
// Scale Y about the floor plane so the base stays seated on the floor rather
// than sinking through it.
const HULL_BASE_Y = -0.33333333333;

fn stretchHullPosition(position: vec3f) -> vec3f {
  return vec3f(
    position.x * HULL_STRETCH.x,
    HULL_BASE_Y + (position.y - HULL_BASE_Y) * HULL_STRETCH.y,
    position.z * HULL_STRETCH.z,
  );
}

// A surface normal transforms by the inverse transpose of the deformation.
// The stretch is diagonal, so that reduces to a component-wise divide.
fn stretchHullNormal(normal: vec3f) -> vec3f {
  return normal / HULL_STRETCH;
}

// The four planes above are authored in unstretched mesh space, so map the ray
// back into that space instead of transforming the planes. An affine change of
// variables preserves the ray parameter exactly, so the distance this yields is
// still a world-space distance and absorption stays correct. This assumes
// `params.model` carries no scale or translation of its own, which holds while
// GLASS_MODEL_MATRIX is `modelMatrix(1, [0, 0, 0])`.
fn hullLocalPoint(point: vec3f) -> vec3f {
  return vec3f(
    point.x / HULL_STRETCH.x,
    HULL_BASE_Y + (point.y - HULL_BASE_Y) / HULL_STRETCH.y,
    point.z / HULL_STRETCH.z,
  );
}

fn hullLocalDirection(direction: vec3f) -> vec3f {
  return direction / HULL_STRETCH;
}
...
@vertex fn vs_main(
  ...
  let meshPosition = mix(params.meshMin, params.meshMax, packed_position.xyz);
  let localPosition = stretchHullPosition(meshPosition);
  let world = params.model * vec4f(localPosition, 1.0);
  var out: VertexOut;
  out.position = params.viewProjection * world;
  out.worldPosition = world.xyz;
  out.worldNormal = normalize(
    (params.model * vec4f(stretchHullNormal(packed_normal.xyz), 0.0)).xyz
  );
  return out;
}
...
fn tetrahedronExitDistance(origin: vec3f, direction: vec3f) -> f32 {
  let localOrigin = hullLocalPoint(origin);
  let localDirection = hullLocalDirection(direction);
  return min(
    min(
      planeExitDistance(localOrigin, localDirection, -V0),
      planeExitDistance(localOrigin, localDirection, -V1),
    ),
    min(
      planeExitDistance(localOrigin, localDirection, -V2),
      planeExitDistance(localOrigin, localDirection, -V3),
    ),
  );
}
