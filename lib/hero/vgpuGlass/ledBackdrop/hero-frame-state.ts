...
export function createHeroFrameState() {
  ...
  const tunableOverrides: Partial<SceneTunables> = {};
  const hero: HeroStateSettings = { ...HERO_STATE_DEFAULTS };

  return {
    setTunables(partial: Partial<SceneTunables>) {
      Object.assign(tunableOverrides, partial);
    },
    resolveFrame(args: ResolveFrameArgs) {
      const { time } = args;
      Object.assign(brush, DEFAULT_BRUSH, BRUSH_RESET, args.patch);
      Object.assign(tunables, TUNABLE_DEFAULTS, tunableOverrides);
