...
@group(0) @binding(1) var ledFloorTexture: texture_2d<f32>;
@group(0) @binding(2) var ledFloorSampler: sampler;
...
fn sampleLedBackdrop(uv: vec2f) -> vec3f {
  let ledUv = vec2(uv.x, 1.0 - uv.y);
  return textureSample(ledFloorTexture, ledFloorSampler, ledUv).rgb;
}
...
@fragment fn fs_main(in: VertexOut) -> @location(0) vec4f {
  ...
  let rd = cameraRay(uv);
  let backdrop = sampleLedBackdrop(uv);
  if (rd.y < -0.0001) {
    ...
    if (floorT > 0.0) {
      ...
      return vec4f(floorColor * floorAo, 1.0);
    }
  }
