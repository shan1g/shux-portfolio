...
const INACTIVE_EDGE_COLOR_MIX = 0.4;
const LUMA_R = 0.2126;
const LUMA_G = 0.7152;
const LUMA_B = 0.0722;
...
function updateLines(
  ...
  for (let i = 0; i < LED_COUNT; i++) {
    ...
    const color = edgeColorForIndex(i);
    writeLed(target, i, clamp01(coverage), color.r, color.g, color.b);
  }
}
...
function updateEdge(
  ...
  for (let i = 0; i < LED_COUNT; i++) {
    const onEdge = i >= start && i < end;
    const brightness = onEdge ? highlight : base;
    const color = onEdge
      ? edgeColorForIndex(i)
      : dimEdgeColor(edgeColorForIndex(i), INACTIVE_EDGE_COLOR_MIX);
    writeLed(target, i, brightness, color.r, color.g, color.b);
  }
}
...
function edgeTintColor(
  ...
  const red = tint?.edgeRedLinear ?? getHeroLedEdgeColor(0);
  const green = tint?.edgeGreenLinear ?? getHeroLedEdgeColor(1);
  const blue = tint?.edgeBlueLinear ?? getHeroLedEdgeColor(2);
  const base = i * LED_FLOATS;
...
function edgeColorForIndex(i: number) {
  const edge = Math.floor(i / LEDS_PER_EDGE);
  return getHeroLedEdgeColor(edge);
}

function dimEdgeColor(color: { r: number; g: number; b: number }, mix: number) {
  const t = clamp01(mix);
  return {
    r: color.r * t,
    g: color.g * t,
